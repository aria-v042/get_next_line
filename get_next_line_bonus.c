...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/07/12 22:34:58 by frodrig2          #+#    #+#             */
/*   Updated: 2026/07/12 22:35:01 by frodrig2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_list	*buffer_list[FD_MAX];
	char			*next_line;

	if (fd < 0 || fd >= FD_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	if (read_into_list(&buffer_list[fd], fd) != 0)
	{
		lst_freeuntil(&buffer_list[fd], NULL);
		return (NULL);
	}
	next_line = extract_line(buffer_list[fd]);
	if (!next_line)
	{
		lst_freeuntil(&buffer_list[fd], NULL);
		return (NULL);
	}
	if (trim_list(&buffer_list[fd]) != 0)
		lst_freeuntil(&buffer_list[fd], NULL);
	return (next_line);
}
